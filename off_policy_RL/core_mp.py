...
def parallel_eval(env, agent, episodes, seed):
    """
    Perform parallel evaluation using vectorized environments.
    """
    env_fns = [lambda: deepcopy(env) for _ in range(episodes)]
    ...
    while not np.all(dones):
        ...
        states, rewards, next_dones, next_truncated, infos = envs.step(actions)
        returns += rewards * ~dones
        dones = dones | next_dones | next_truncated
    agent.train()
    envs.close()
    return np.mean(returns), np.std(returns)
...
def train(cfg, seed, log_dict, idx, logger, barrier):
    ...
    for step in range(1, cfg.timesteps + 1):
        ...
        if step % cfg.eval_interval == 0:
            ...
            if use_parallel_eval:
                eval_mean, eval_std = parallel_eval(eval_env, agent=agent, episodes=cfg.eval_episodes, seed=seed)
            else:
    ...
    if use_parallel_eval:
        eval_mean, eval_std = parallel_eval(env, agent=agent, episodes=cfg.eval_episodes, seed=seed)
    else:

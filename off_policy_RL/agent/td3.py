...
class TD3Agent(DDPGAgent):
    ...
    def get_Qs(self, 
        ...
        action_low = self.actor_target.action_space.low.to(self.device)
        action_high = self.actor_target.action_space.high.to(self.device)
        next_action = (self.actor_target(next_state) + noise).clamp(action_low, action_high)
        
        Q = self.critic_net(state, action)
        Q2 = self.critic_net_2(state, action)
        
        max_next_Q = torch.min(
            self.critic_target(next_state, next_action),
            self.critic_target_2(next_state, next_action)
        )
        Q_target = reward + (1 - done) * self.gamma * max_next_Q
    ...
    def update(self, batch, weights=None):
        ...
        if not (self.train_step % self.policy_update_interval) % self.target_update_interval:

...
class DDPGAgent:
    ...
    def get_Qs(self, 
        ...
        max_next_Q = self.critic_target(next_state, self.actor_target(next_state))
    ...
    def get_action(self, 
        ...
        if sample:
            action += self.epsilon_schedule(self.train_step) * np.random.randn(*action.shape)
        action = np.clip(action, self.actor_net.action_space.low.cpu().numpy(), self.actor_net.action_space.high.cpu().numpy())

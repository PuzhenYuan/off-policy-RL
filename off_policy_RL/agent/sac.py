...
class SACAgent(TD3Agent):
    ...
    def get_Qs(self, 
        ...
        with torch.no_grad():
            ...
            Q_target_1 = self.critic_target(next_state, next_action)
            Q_target_2 = self.critic_target_2(next_state, next_action)
            Q_target = reward + (1 - done) * self.gamma * (torch.min(Q_target_1, Q_target_2) - self.log_alpha.exp() * next_log_prob)
    ...
    def get_actor_loss(self, 
        ...
        action, action_log_prob = self.actor_net.evaluate(state)
        Q = self.critic_net(state, action)
        actor_loss = (self.log_alpha.exp() * action_log_prob - Q).mean()
